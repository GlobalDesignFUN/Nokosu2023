...
class ProfileSerializer(ModelSerializer):
    class Meta:
        model = Profile
        fields = '__all__' 
 #MAKE profile pic update and delete only and, info pic insert and delete    
    def create(self, validated_data):
        storage.child("ProfilePics/" + validated_data['user'].username).put(validated_data.get('photo'))
        instance = super(ProfileSerializer, self).create(validated_data)
        return instance
    
    def update(self, instance, validated_data):
        print(instance.user)
        delit = default_storage.delete(os.path.basename(instance.photo.name))
        print(delit)
        storage.child("ProfilePics/" + instance.user.username).put(validated_data.get('photo'))
